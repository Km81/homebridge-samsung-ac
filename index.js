...
class SamsungAirco {
    constructor(log, config) {
        ...
        this.log.info(`[${this.name}] 플러그인 초기화 중... 버전 1.4.8`);
        this.log.info(`- IP: ${this.ip}, Device Index: ${this.deviceIndex}, Cache: ${this.cacheDuration}ms`);
            ...
            const req = https.request(options, (res) => {
                ...
                res.on('end', () => {
                    try { 
                        ...
                        this.log.debug(`[RESPONSE] <-- ${method} ${path} 성공`);
                        resolve(parsedBody);
                    } 
                    catch (e) { reject(e); }
                });
    ...
    async getAndUpdateStateInBackground(caller = '배경 업데이트') {
        if (this.isFetching) {
            this.log.debug(`[${caller}] 배경 업데이트 요청이 있었으나, 이미 다른 업데이트가 진행 중입니다.`);
            return;
        }

        this.log.info(`[${caller}] 배경에서 최신 상태를 가져옵니다...`);
        ...
        try {
            ...
            if (responseData && responseData.Devices && responseData.Devices[this.deviceIndex]) {
                ...
                this.log.info('배경 업데이트 성공. 홈킷 UI에 상태를 푸시합니다.');
                this.pushStateToHomeKit();
            } else {
                this.log.warn('기기로부터 유효하지 않은 응답을 받았습니다.');
            }
        } catch (error) {
    ...
    pushStateToHomeKit() {
        ...
        this.log.debug("--- 홈킷 UI 푸시 업데이트 시작 ---");
        ...
        const powerState = this.deviceState.Operation.power === "On" ? 1 : 0;
        this.log.debug(`[PUSH] 전원: ${this.deviceState.Operation.power}`);
        this.aircoSamsung.updateCharacteristic(Active, powerState);

        const currentTemp = this.deviceState.Temperatures[0].current;
        this.log.debug(`[PUSH] 현재 온도: ${currentTemp}°C`);
    ...
    async sendCommand(endpoint, data) {
        ...
        try {
            this.log.info(`[SET] 명령어 전송 시작: ${endpoint} 데이터: ${JSON.stringify(data)}`);
            await this._request('PUT', fullEndpoint, data);
            this.log.info(`[SET] 명령어 전송 성공.`);
            this.lastStateUpdate = 0;
    ...
    getServices() {
        ...
        this.aircoSamsung.getCharacteristic(Active)
            .on('get', cb => this.handleGet(cb, '전원', s => s.Operation.power === "On" ? 1 : 0))
        ...
        this.aircoSamsung.getCharacteristic(SwingMode)
            .on('get', cb => this.handleGet(cb, '스윙/무풍 모드', s => {
                return (this.swingModeType === 'wind')
                    ? (s.Wind.direction === "Up_And_Low" ? 1 : 0)
                    : (s.Mode.options.includes("Comode_Nano") ? 1 : 0);
            }))
            .on('set', this.setSwingMode.bind(this));
        
        this.aircoSamsung.getCharacteristic(LockPhysicalControls)
            .on('get', cb => this.handleGet(cb, '자동 청소', s => s.Mode.options.includes("Autoclean_On") ? 1 : 0))
    ...
    async setActive(value, callback) {
        this.log.info(`[SET] '전원'을(를) ${value ? 'On' : 'Off'}(으)로 설정 요청.`);
        try {
            this.aircoSamsung.updateCharacteristic(Characteristic.Active, value);
            await this.sendCommand('', { Operation: { power: value ? 'On' : 'Off' } });
            callback(null);
        } catch (error) {
            this.log.error('[SET] \'전원\' 상태 설정 실패:', error);
            callback(error);
        }
    }
    async setTargetTemperature(value, callback) {
        this.log.info(`[SET] '목표 온도'를(을) ${value}°C(으)로 설정 요청.`);
        try {
            this.aircoSamsung.updateCharacteristic(Characteristic.CoolingThresholdTemperature, value);
    ...
    async setSwingMode(value, callback) {
        ...
        this.log.info(`[SET] '${mode}'을(를) ${value ? 'On' : 'Off'}(으)로 설정 요청.`);
        try {
            this.aircoSamsung.updateCharacteristic(Characteristic.SwingMode, value);
    ...
    async setLockPhysicalControls(value, callback) {
        this.log.info(`[SET] '자동 청소'을(를) ${value ? 'On' : 'Off'}(으)로 설정 요청.`);
        try {
            this.aircoSamsung.updateCharacteristic(Characteristic.LockPhysicalControls, value);
            await this.sendCommand('/mode', { options: [value ? 'Autoclean_On' : 'Autoclean_Off'] });
            callback(null);
        } catch (error) { callback(error); }
    }
    async setTargetHeaterCoolerState(value, callback) {
        this.log.info(`[SET] '목표 운전 모드'를(을) ${value}(으)로 설정 요청.`);
        try {
