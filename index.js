...
       } else {
           // 만약 '켜기'를 눌렀다면
           this.log.info('Turning on power, then setting mode to "DryClean" after 2 seconds...');

           // ✅ 수정된 로직 시작
           // 1. 먼저 전원을 'On'으로 설정하라는 명령을 보냅니다.
           this.sendCommand('', { Operation: { power: 'On' } })
               // 2. 전원 켜기 명령이 성공하면, 2초를 기다립니다.
               .then(() => {
                   this.log.info('Power is on. Waiting for 2 seconds before setting mode...');
                   return new Promise(resolve => setTimeout(resolve, 2000));
               })
               // 3. 2초 뒤에, 운전 모드를 'DryClean'으로 설정하라는 명령을 보냅니다.
               .then(() => this.sendCommand('/mode', { modes: ['DryClean'] }))
               // 4. 모든 명령이 성공적으로 끝나면 Homebridge에 성공을 알립니다.
               .then(() => {
                   this.log.info('Successfully turned on and set mode to DryClean.');
...
                   callback(null);
               })
               // 5. 중간에 어느 단계라도 실패하면 에러를 보고합니다.
               .catch(error => {
                   this.log.error('Failed to turn on and set mode:', error);
                   callback(error);
               });
       }
   }
