...
class SamsungAirco {
  constructor(log, config) {
    ...
    this.log.info(`[${this.name}] 플러그인 초기화 중... 버전 1.6.3`);
    ...
    // 15초마다 심장박동 이벤트 전송 (모든 특성)
    setInterval(() => this._sendHeartbeat(), 15000);
  }
  ...
  _sendHeartbeat() {
    ...
    if (!s || !s.Operation) {
      this.log.warn(`[${this.name}] [HEARTBEAT] 상태 미설정, 이벤트 재전송 스킵`);
      return;
    }
    ...
    this.log.info(`[${this.name}] [HEARTBEAT] 스윙/무풍 모드 이벤트 재전송: ${swing?'켜짐':'꺼짐'}`);
  ...
  async getAndUpdateStateInBackground(caller) {
    if (this.isFetching) {
      this.log.debug(`[${this.name}] '${caller}' 요청: 이미 업데이트 중`);
      return;
    }
    this.log.info(`[${this.name}] '${caller}' 요청: 기기 상태 가져오는 중...`);
    ...
    try {
      ...
      if (device) {
        ...
        this.log.info(`[${this.name}] '${caller}' 성공. UI에 푸시합니다.`);
        this._sendHeartbeat();      // 변경된 상태 즉시 푸시
      } else {
        this.log.warn(`[${this.name}] '${caller}' 기기 인덱스 ${this.deviceIndex} 없음`);
      }
    } catch (err) {
      this.log.error(`[${this.name}] '${caller}' 실패: ${err.message}`);
    } finally {
      this.isFetching = false;
    }
  }
  ...
  handleGet(callback, caller, extractor) {
    ...
    this.log.info(`[${this.name}] [GET] '${caller}' 요청. 캐시된 값 (${val}) 응답`);
    callback(null, val);
  }
